...
variable "google_sheet_key" {
  description = "The key from the Google Sheet URL (e.g., 1GuqpdVRx8_...)"
  type        = string
...
data "archive_file" "lambda_zip" {
  type        = "zip"
  source_file = "${path.module}/main.py" # Points to the new .py file
  output_path = "${path.module}/lambda_function.zip"
}

# 2. IAM Role
resource "aws_iam_role" "lambda_role" {
  name = "gsheet-rightsizing-lambda-role"
...
resource "aws_iam_policy" "lambda_policy" {
  name = "gsheet-rightsizing-policy"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
resource "aws_lambda_function" "rightsizing_lambda" {
  ...
  function_name    = "gsheet-ec2-rightsizing-reporter" # New function name
  role             = aws_iam_role.lambda_role.arn
  handler          = "main.lambda_handler" # Points to main.py -> lambda_handler
  ...
  layers = [var.gspread_layer_arn] # Attaches the gspread libraries
}

# 5. Schedule
resource "aws_cloudwatch_event_rule" "schedule" {
  name                = "gsheet-weekly-wednesday-report" # New rule name
